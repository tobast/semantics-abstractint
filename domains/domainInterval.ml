open Cfg
open Abstract_syntax_tree

type t = ValueInterval.t VarMap.t

let init vars =
	List.fold_left (fun cur var ->
		VarMap.add var (ValueInterval.const (Z.zero)) cur) VarMap.empty vars

let bottom = VarMap.empty

let rec evalIntExpr dom = function
| CFG_int_unary(unop, expr) ->
        ValueInterval.unary (evalIntExpr dom expr) unop
| CFG_int_binary(binop,e1,e2) ->
        ValueInterval.binary (evalIntExpr dom e1) (evalIntExpr dom e2) binop
| CFG_int_var(v) ->
        (try VarMap.find v dom
        with Not_found -> ValueInterval.bottom)
| CFG_int_const(c) -> ValueInterval.const c
| CFG_int_rand(low,high) -> ValueInterval.rand low high

let assign dom var expr =
    VarMap.add var (evalIntExpr dom expr) dom
    
let join d1 d2 =
    VarMap.merge (fun key v1 v2 -> (match v1,v2 with
        | None,None -> None
        | None,v | v,None -> v
        | Some sv1, Some sv2 -> Some (ValueInterval.join sv1 sv2))) d1 d2
        
let meet =
    VarMap.merge (fun key v1 v2 -> (match v1,v2 with
        | None, None | None, Some _ | Some _, None -> None
        | Some sv1, Some sv2 -> Some (ValueInterval.meet sv1 sv2)))

exception NotElimFail
let guard dom expr =
    let rec guardBool = function
    | CFG_bool_unary(unop,exp) ->
        (match unop with
        | AST_NOT -> raise NotElimFail
        )
    | CFG_bool_binary(binop,e1,e2) ->
        let dom1 = guardBool e1
        and dom2 = guardBool e2 in
        
        (match binop with
        | AST_AND -> meet dom1 dom2
        | AST_OR -> join dom1 dom2
        )
    | CFG_compare(cmp,e1,e2) ->
        let iv1 = evalIntExpr dom e1
        and iv2 = evalIntExpr dom e2 in
        
        let left,right = ValueInterval.compare iv1 iv2 cmp in
        ignore left ; ignore right ;
        assert false
    | CFG_bool_const(b) ->
        (match b with
        | false -> bottom
        | true -> dom
        )
    | CFG_bool_rand ->
        bottom
    in
    
    guardBool (Helpers.notElim expr)
    
let widen =
    VarMap.merge (fun key v1 v2 -> (match v1,v2 with
        | None,None -> None
        | Some v, None | None, Some v -> Some v
        | Some v1, Some v2 -> Some (ValueInterval.widen v1 v2)))

let subset d1 d2 =
    VarMap.fold (fun key v cur -> match cur with
        | false -> false
        | true ->
                (try
                    ValueInterval.subset (VarMap.find key d1) v
                with Not_found ->
                    true))
        d2 true

let is_bottom = VarMap.is_empty

let print chan dom =
    VarMap.iter (fun k v ->
        Cfg_printer.print_var chan k;
        Printf.fprintf chan ": ";
        ValueInterval.print chan v;
        Printf.fprintf chan "\n") dom

